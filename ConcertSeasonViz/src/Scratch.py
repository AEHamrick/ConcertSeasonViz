libraryHandle = open(inputXML)
    ...
    topLevelDict = libraryXML.find("dict").find("dict").find_all("dict")
    #print topLevelDict[0]
        
    #topLevelDict = libraryXML.find("dict").contents[25]
    #print topLevelDict
    #print type(topLevelDict)
    
    for tag in topLevelDict:
        print type(tag)
        #for innerTag in tag
        #    print type(innerTag)
...
currentArtistName = tag.find_all("key",string="Artist")[0].next_siblings().string()
        currentGenreName = tag.find_all("key",string="Genre")[0].next_siblings().string()
